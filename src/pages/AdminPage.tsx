...
import { Upload, LogOut, Check, AlertCircle, Film, Trash2, Link } from 'lucide-react';
...
export default function AdminPage() {
  ...
  async function fetchCurrentVideo() {
    ...
    if (data?.hero_video_url) {
      setCurrentVideoUrl(data.hero_video_url);
      setExternalUrl(data.hero_video_url);
    }
  }
  ...
  async function handleSaveExternalUrl(e: React.FormEvent) {
    e.preventDefault();
    ...
    const { error } = await supabase
      .from('site_settings')
      .update({ hero_video_url: externalUrl.trim(), updated_at: new Date().toISOString() })
      .eq('id', 1);

    if (error) {
      setUploadError('Ошибка сохранения: ' + error.message);
    } else {
      setCurrentVideoUrl(externalUrl.trim());
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-white font-semibold text-lg">Ссылка на видео</h2>
          </div>
          <p className="text-gray-400 text-sm mb-4">
            Вставьте прямую ссылку на MP4-файл. Рекомендуем хостить видео на российских сервисах
            для стабильной работы без VPN из России.
          </p>
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 mb-4">
            <p className="text-amber-400 text-xs leading-relaxed">
              Supabase Storage может быть недоступен из РФ. Используйте российские хостинги:
              Selectel, Timeweb Cloud, Yandex Cloud, VK Cloud, или GitHub Releases (бесплатно).
            </p>
          </div>

          <form onSubmit={handleSaveExternalUrl} className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none text-sm"
              />
            </div>
            <button
              type="submit"
              disabled={savingUrl || !externalUrl.trim()}
              className="px-5 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-semibold rounded-lg transition-colors text-sm whitespace-nowrap"
            >
              {savingUrl ? '...' : 'Сохранить'}
            </button>
          </form>
        ...
        {/* Upload area */}
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 sm:p-8">
          <h2 className="text-white font-semibold text-lg mb-2">Или загрузите файл</h2>
          <p className="text-gray-400 text-sm mb-6">
            Загрузите видео в Supabase Storage. Формат: MP4, до 50 МБ.
          </p>
